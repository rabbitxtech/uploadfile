...
// Task5 #7 — Web Push. The backend (web-push) sends an encrypted JSON payload
// { title, body, link, type }; show it as a system notification.
self.addEventListener('push', (event) => {
  let data = {};
  try {
    data = event.data ? event.data.json() : {};
  } catch {
    data = { title: 'Uploader', body: event.data ? event.data.text() : '' };
  }
  const title = data.title || 'Uploader';
  event.waitUntil(
    self.registration.showNotification(title, {
      body: data.body || '',
      icon: '/icon-192.png',
      badge: '/icon-192.png',
      tag: data.type || undefined, // collapse repeats of the same kind
      data: { link: data.link || '/' },
    }),
  );
});
